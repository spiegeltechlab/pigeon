...
suite('diff', (test) => {
    ...
    test('object property primitive', async () => {
        assert.deepEqual(diff({ id: 1, title: 'hello' }, { id: 1 }), [
    ...
    test('primitive array append (replace)', async () => {
        assert.deepEqual(diff(['a', 'b'], ['a', 'b', 'c']), [
            {
                op: 'replace',
                path: '/',
                value: ['a', 'b', 'c'],
    ...
    test('primitive array prepend (replace)', async () => {
        assert.deepEqual(diff(['a', 'b'], ['z', 'a', 'b']), [
            {
                op: 'replace',
                path: '/',
                value: ['z', 'a', 'b'],
    ...
    test('primitive array replace', async () => {
        assert.deepEqual(diff(['a', 'b', 'c'], ['a', 'b', 'x']), [
            {
                op: 'replace',
                path: '/',
                value: ['a', 'b', 'x'],
            ...
            [
                {
                    op: 'remove',
                    path: '/[24]',
                    _prev: { id: 24, name: 'boise', value: 239 },
                },
            ]
        );
    });
    ...
    test('null to obj', async () => {
        assert.deepEqual(
            diff({ name: null }, { name: { first: 'Joseph', last: 'Biden' } }),
            [
                {
                    op: 'replace',
                    path: '/name',
                    value: { first: 'Joseph', last: 'Biden' },
                    _prev: null,
                },
            ]
        );
    ...
    test('obj to null', async () => {
        assert.deepEqual(
            diff({ name: { first: 'Joseph', last: 'Biden' } }, { name: null }),
            [
                {
                    op: 'replace',
                    path: '/name',
                    value: null,
                    _prev: { first: 'Joseph', last: 'Biden' },
                },
            ]
        );
    });
    ...
    test('array order shuffle literals', async () => {
        assert.deepEqual(diff(['def', 'abc', 'ghi'], ['abc', 'def', 'ghi']), [
            {
                op: 'replace',
                path: '/',
                value: ['abc', 'def', 'ghi'],
    ...
    test('primitive array remove and re-order literal', async () => {
        assert.deepEqual(diff(['abc', 'def', 'hij'], ['hij', 'def']), [
            {
                op: 'replace',
                path: '/',
                value: ['hij', 'def'],
            ...
            [
                {
                    op: 'replace',
                    path: '/',
                    value: ['hij', 'nop', 'def', '000', 'klm'],
                    _prev: ['abc', 'def', '000', 'hij', 'klm', 'nop'],
                },
            ]
        );
    });
