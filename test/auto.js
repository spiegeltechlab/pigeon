...
suite('auto', (test) => {
    test('from', async () => {
        ...
        assert.deepEqual(history[0].diff, [
            { op: 'add', path: '/cards', value: [] },
        ]);
    ...
    test('load', async () => {
        const doc = AutoPigeon.load(
            JSON.stringify({'meta': {'history': []}, 'data': {'message': 'salut!'}})
        );
    ...
    test('load truncated history', async () => {
        ...
        assert.equal(
            beforeDoc.meta.history.length,
            101,
            'history length is 101'
        );
        assert.equal(
            Object.keys(beforeDoc.meta.changeIds).length,
            101,
            'changeIds length is 101'
        );
        ...
        assert.equal(
            afterDoc.meta.history.length,
            10,
            'history length was truncated'
        );
        assert.equal(
            Object.keys(afterDoc.meta.changeIds).length,
            10,
            'changeIds length was truncated'
        );
        ...
        assert.equal(
            hasMissingChangeId,
            false,
            'all items in history have a change_id in the changeIds object'
        );
        ...
        for (const changeId of Object.keys(afterDoc.meta.changeIds)) {
            const historyItem = afterDoc.meta.history.find(
                (h) => h.change_id == changeId
            );
            if (!historyItem) {
        ...
        assert.equal(
            hasMissingHistoryItem,
            false,
            'all changeIds have an associated history item'
        );

        assert.deepEqual(diff, [
            { op: 'replace', path: '/count', value: 100, _prev: 99 },
        ]);
    ...
    test('merge', async () => {
        ...
        doc1 = AutoPigeon.change(doc1, (d) =>
            d.cards.push({ title: 'do refactoring', id: 1 })
        );

        let doc2 = AutoPigeon.clone(doc1);
        doc2 = AutoPigeon.change(doc2, (d) =>
            d.cards.push({ title: 'do quality assurance', id: 2 })
        );
